...
| LEDs | 12 × WS2812B-5050 addressable RGB (one per key; ~3.5 mm emitting window) |
| LED placement | Uniform 13.75 mm pitch (165 mm octave = 6.5″ standard; one per chromatic key), single row |
...
LED X-coordinates (mm from octave start), uniform 13.75 mm pitch (octave = 165 mm = the 6.5″ industry standard and the measured average across modern brands; real range 164–166 mm): 6.875, 20.625, 34.375, 48.125, 61.875, 75.625, 89.375, 103.125, 116.875, 130.625, 144.375, 158.125. Center-anchor the strip so cross-piano drift splits both ways (standard 165 mm pianos ≈0; Yamaha 164 mm or a 166 mm piano ≤ ±2.46 mm over 5 octaves). Uniform spacing also trades per-key centering (C, F, B ~5–7 mm; others within ~3 mm), recovered in firmware by the note→LED map. See hardware/octave-flex-cell/KEY_SPACING_FIT.md.
...
|---|---|---|
| LED type | WS2812B-5050 addressable RGB | Full color, single data wire, per-key control; ~3.5 mm emitting window, hand-solderable |
| LED count | 60 — one per key, 5 octaves | Exact per-key guidance |
| LED form factor | Custom flex PCBs | Off-the-shelf strip densities (60/m, 144/m) don't hit the 13.75 mm key pitch; custom = exactly one LED per key |
...
## 5. Power Budget & LED Brightness

**LED type reality:** the LEDs are **WS2812B 5050** (LCSC C2761795, 5.0×5.4 mm), not 2020 — each draws up to **~60 mA** at full white. 60 LEDs full white = **3.6 A absolute max** (never run uncapped).

**Protection (in priority order):**
1. **MANDATORY firmware cap — non-negotiable:** `FastLED.setMaxPowerInVoltsAndMilliamps(5, 900);`
   Auto-scales brightness so total LED draw never exceeds **900 mA**, regardless of pattern. This is the real hardware safety net — it protects the JST-SH connector (1 A/pin), the 0.8 mm flex +5 V trace, and the inter-board cable. Keep it in *every* build.
2. **App rule:** max ~10 LEDs lit simultaneously (10 × 60 mA = 0.6 A at 100 %).
3. **Global brightness limit** (see below).

**Brightness guidance (current ≈ 3.6 A × Brightness/255, all 60 white):**

| Global brightness | All-60-white draw | Notes |
|---|---|---|
| 255 (100 %) | 3.6 A | ❌ never, unless ≤10 LEDs are ever lit |
| 128 (50 %) | ~1.8 A | OK for normal lessons (few keys lit); power cap auto-dims the rare all-on frame |
| **96–128 (≈38–50 %)** | — | **recommended default** for lesson use |
| 64 (25 %) | ~0.9 A | safe even with *all 60* lit white, without relying on the cap |

**Bottom line:** set default global brightness to **~50 % (128)** and **always** keep the 900 mA `setMaxPowerInVoltsAndMilliamps` cap. If you ever drive patterns where most/all LEDs light at once, cap brightness at **25 % (64)** as well. Do not exceed these without widening the +5 V copper and using a higher-rated connector.

**System total (normal use):** ~10 LEDs (0.6 A) + ESP32-S3 + BLE peak (~0.5 A) ≈ **1.1 A** — comfortably within the USB-C 5 V/3 A supply.
