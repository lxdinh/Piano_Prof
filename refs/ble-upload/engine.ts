...
import {
  cmdSetMulti, cmdClearAll, cmdCommit,
} from '../ble/protocol';
        ...
        case 'quiz': {
          ...
          if (passed) {
            ...
            if (ble.phase === 'CONNECTED') {
              // Fire-and-forget: the burst is a ~250 ms animation driven from
              // the app now (the firmware has no pattern engine), and the
              // lesson should not stall waiting for it.
              void ble.runEffect('successBurst').catch(() => { /* ignore */ });
            }
          }
          setState((s) => ({
            ...s,
