...
import { CAL_STEPS } from '../ble/constants';
import { EffectName } from '../ble/effects';
...
function ConnectedScreen({
  ...
  onTestLights: () => void;
  onDisconnect: () => void;
  runEffect: (name: EffectName) => Promise<void>;
}) {
  ...
  async function handleTestLights() {
    ...
    try {
      ...
      /* module may have dropped — the UI just stops animating */
    }
    onTestLights();
...
export default function BLEPairingScreen({
  ...
  // A factory-fresh module has the OTA service and nothing else — no lights to
  // test and nothing to calibrate — so send it straight to the updater.
  useEffect(() => {
    if (ble.phase === 'CONNECTED' && ble.isRecovery) {
      navigation.navigate('FirmwareUpdate');
    }
  }, [ble.phase, ble.isRecovery, navigation]);
